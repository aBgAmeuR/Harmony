export const getMonthIndex = (monthStr: string, months: string[]): { monthIndex: number; year: number } => {
  const match = monthStr.match(/^([A-Za-z]+)\s+(\d{4})$/);
  if (!match) return { monthIndex: -1, year: 0 };
  const [, monthName, yearStr] = match;
  const monthIndex = months.findIndex((m) => m === monthName);
  return { monthIndex, year: parseInt(yearStr) };
};
...
export const generateMonthRange = (
  ...
  const result: string[] = [];
  let y = first.year, m = first.monthIndex;
