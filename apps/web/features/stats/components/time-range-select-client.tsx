...
export const TimeRangeSelectClient = ({ timeRange }: TimeRangeSelectClientProps) => {
    const loader = useTopLoader()
    const queryClient = useQueryClient()
    const { mutate, isPending } = useMutation({
        mutationFn: async (newTimeRange: TimeRange) => await setTimeRangeAction(newTimeRange),
        onError: () => toast.error("Failed to update time range, please try again later"),
