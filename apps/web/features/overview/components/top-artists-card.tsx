...
export const TopArtistsCard = async ({ data, userId, isDemo }: TopArtistsCardProps) => {
    return (
        <Card className="col-span-1 pb-2">
            ...
            <CardContent>
                <Suspense fallback={<ListSkeleton length={5} />}>
                    <RankingArtists data={data} userId={userId} isDemo={isDemo} limit={5} config={{ actionHref: null }} />
                </Suspense>
            </CardContent>
        </Card>
    )
