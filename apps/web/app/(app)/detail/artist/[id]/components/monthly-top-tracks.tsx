...
import { MonthlyTrackData } from "~/services/details/get-monthly-top-tracks";
...
export function MonthlyTopTracks({ dataPromise, artistNamePromise }: MonthlyTopTracksProps) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            No monthly listening data available for {artistName || 'this artist'}.
          </CardDescription>
        </CardHeader>
  ...
  const currentMonth = data.find((month) => month.month === format(date, "MMMM yyyy")) || data[0];
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="space-y-1">
            ...
            <CardDescription>
              Your most played tracks by {artistName || 'this artist'} during {month}.
            </CardDescription>
          </div>
          <MonthSelector
            date={date}
        ...
        <CardContent className="flex flex-col p-4 pb-0">
          {tracks.length > 0 ? (
              ...
              <div key={track.id} className="flex flex-col">
                <MusicItemCard
                  item={{
                    ...
                    description: (<TrendBadge trend={track.trend} previousRank={track.previousRank} />)
                  }}
                  rank={index + 1}
                />
            ...
            <div className="flex flex-col items-center justify-center p-6 text-center text-muted-foreground">
              <Music4 className="mb-2 size-8" />
              <p>No tracks played for {artistName || 'this artist'} during {month}.</p>
            </div>
          )}
        </CardContent>
      </Card>
...
function MonthSelector({ date, setDate, minDate, maxDate }: { date: Date; setDate: (date: Date) => void; minDate: Date; maxDate: Date; }) {
  return (
    <ButtonGroup size="sm">
      <Button size="sm" variant="outline" onClick={() => setDate(new Date(date.getFullYear(), date.getMonth() + 1))} disabled={_.gte(date.getTime(), maxDate.getTime())}>
        <ArrowLeft className="size-4" />
      </Button>
      <Popover>
        <PopoverTrigger asChild>
          <Button size="sm" variant="outline" className={cn("w-[86px] justify-start text-left font-normal", !date && "text-muted-foreground")}>
      ...
      <Button size="sm" variant="outline" onClick={() => setDate(new Date(date.getFullYear(), date.getMonth() - 1))} disabled={_.lte(date.getTime(), minDate.getTime())}>
