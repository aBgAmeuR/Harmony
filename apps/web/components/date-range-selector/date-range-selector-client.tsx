...
export const DateRangeSelectorClient = ({
	dateRange,
	minMaxDateRange,
	isDemo,
}: DateRangeSelectorClientProps) => {
	...
	const {
		...
		setPreset,
		PRESETS,
	} = useDateRange({ dateRange, minMaxDateRange });
	const { mutate, isPending } = useMutation({
		mutationFn: async () =>
			await setDateRangeAction(
				selectedDateRange.from,
				preset === "custom"
					? DateUtils.addMonths(selectedDateRange.to, 1)
					: selectedDateRange.to,
			),
		onError: () =>
			toast.error("Failed to update date range, please try again later"),
