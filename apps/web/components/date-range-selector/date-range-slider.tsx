...
export const DateRangeSlider = ({ selectedDateRange, setDateRange }: DateRangeSliderProps) => {
    ...
    const findClosestIndex = useCallback((date: Date) => {
        ...
        <div>
            <div className="flex h-12 w-full items-end px-3" aria-hidden="true">
                {data.map((item, i) => (
                    ...
                    >
                        <span className={cn("size-full bg-primary/20", DateUtils.isDateInRange(item.date, data[sliderValue[0]].date, data[sliderValue[1]]?.date) && "bg-primary/50")} />
                    </div>
                ))}
            </div>
            <Slider
                ...
                max={data.length - 1}
                min={0}
                onValueChange={(value) => setDateRange({ from: data[value[0]].date, to: data[value[1]].date })}
