...
function ChartCard({ className, children }: PropsWithChildrenAndClassName) {
	return <Card className={cn("pt-0", className)}>{children}</Card>;
}
...
function ChartCardHeader({
	...
	return (
		<CardHeader
			className={cn(
				"flex flex-col items-stretch space-y-0 p-0 sm:flex-row [.border-b]:pb-0",
				{ "border-b": showSeparator },
				className,
			)}
		>
			<div
				className={cn(
					"flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-5",
				)}
			>
...
function ChartCardHeaderContent({
	className,
	children,
	title,
	description,
...
export { ChartCard, ChartCardHeader, ChartCardHeaderContent, ChartCardContent };
